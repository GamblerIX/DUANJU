...
class DownloadWorker(QThread):
    ...
    task_progress = Signal(str, float, int, int)  # task_id, progress, downloaded, total
    task_completed = Signal(str)  # task_id
    task_failed = Signal(str, str)  # task_id, error
    video_info_fetched = Signal(str, str)  # task_id, video_url
...
class DownloadService(QObject):
    ...
    def cancel(self) -> None:
        """取消所有下载"""
        logger.info("Cancelling download service...")
        if self._worker:
            self._worker.cancel()
            # 等待线程结束，如果超时则强制终止
            if not self._worker.wait(5000):
                logger.warning("Download worker timeout, terminating...")
                self._worker.terminate()
                self._worker.wait()
            self._worker = None
            self._is_running = False
            logger.info("Download service cancelled")
