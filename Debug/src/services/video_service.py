...
class VideoService(QObject):
    ...
    async def _do_fetch_episodes(self, book_id: str):
        ...
        if response.success:
            ...
            logger.error(f"Failed to fetch episodes: {response.error}")
    ...
    async def _do_fetch_video_url(self, video_id: str, quality: str):
        ...
        if response.success:
            ...
            logger.error(f"Failed to fetch video URL: {response.error}")
    ...
    def _on_error(self, e: Exception) -> None:
        """处理错误"""
        logger.log_service_error("VideoService", "request", e)
        self._is_loading = False
