...
from qfluentwidgets import ScrollArea, Pivot, FlowLayout, InfoBar, InfoBarPosition
...
class CategoryInterface(ScrollArea):
    ...
    def _on_categories_loaded(self, categories: List[str]) -> None:
        ...
        for i, category in enumerate(display_categories):
            self._pivot.addItem(routeKey=f"cat_{i}_{category}", text=category,
    ...
    def _on_error(self, error) -> None:
        self._loading.hide()
        self._flow_widget.show()
        InfoBar.error(title="加载失败", content=error.message, orient=Qt.Orientation.Horizontal,
